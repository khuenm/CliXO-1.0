...
double calculateClusterWeight(boost::dynamic_bitset<unsigned long> cluster,
                              nodeDistanceObject &nodeDistances) {
  //        vector <double> edgeWeights;
  double sumEdgeWeights = 0;
  unsigned nEdges = cluster.count() * (cluster.count() - 1) / 2;
  for (unsigned long i = cluster.find_first(); i < cluster.size() - 1;
       i = cluster.find_next(i)) {
    for (unsigned long j = cluster.find_next(i); j < cluster.size();
         j = cluster.find_next(j)) {
...
public:
  ClusterBitset(const boost::dynamic_bitset<unsigned long> &cluster,
                unsigned long &clustID, double thisClusterWeight = 0) {
    elements = cluster;
    isClusterNew = true;
    isClusterAddedToExplain =
        false; // comment things that may be uncessary later
  ...
  inline double getModularity() { return clusterModularity; }

  inline void setWeight(const double &weight) { clusterWeight = weight; }

  inline void setModularity(const double &q) { clusterModularity = q; }
  ...
  inline void setNew() { isClusterNew = true; }
...
public:
  currentClusterClassBitset(unsigned numNodesHere, double FirstWeight = 1,
                            double thisAlpha = 0) {
  ...
  inline void setLargestCluster(unsigned long size) { largestCluster = size; }

  inline unsigned long getLargestCluster() { return largestCluster; }
  ...
  /**/
  unsigned long
  addCluster(const boost::dynamic_bitset<unsigned long> &newCluster,
             nodeDistanceObject &nodeDistances,
             vector<string> &nodeIDsToNames) {
    ...
    currentClusters[newID] =
        ClusterBitset(newCluster, newID, curWeight); // think about curWeight
  ...
  inline void deleteCluster(const unsigned long &clusterToDelete,
    ...
    // TODO: should also delete edge from clusterGraph if this is the only
    // cluster explaining that edge
  ...
  /*inline*/ void
  setClusterValid(const boost::dynamic_bitset<unsigned long> &cluster,
                  graph_undirected_bitset &clusterGraph) {
    ...
    for (unsigned i = 0; i < clusterElems.size() - 1; ++i) {
      for (unsigned j = i + 1; j < clusterElems.size(); ++j) {
        ...
        //                if (!clusterGraph.isEdge(clusterElems[i],
        //                clusterElems[j])) {//add edges to clusterGraph
        //                   clusterGraph.addEdge(clusterElems[i],
        //                   clusterElems[j]);
        //                }
      }
    }
  ...
  inline void setClusterValid(unsigned long clusterID,
    ...
    // don't deal with hidden clusters here. hidden clusters should be deleted
    // alltogether in once
    return;
  }
  ...
  inline vector<unsigned long>::iterator
  clustersWithNodeBegin(unsigned nodeID) {
  ...
  inline void
  addClustersToExplanations(vector<unsigned long> &sortedNewClusters) {
  ...
  /*inline*/ void
  prepareForValidityCheck(vector<unsigned long> &sortedNewClusters) {
  ...
  inline bool checkClusterValidity(unsigned long id) {
    ...
    for (vector<unsigned>::const_iterator it1 = cluster.begin();
      ...
      ++it2;
      for (; it2 != cluster.end(); ++it2) {
        if (edgesToClusters[*it1][*it2] ==
            1) { // this edge only appears in one cluster. may use realedges so
                 // fewer necessary clusters?
          return true;
        }
      }
    }
  ...
  inline const boost::dynamic_bitset<unsigned long> &
  getElements(unsigned long id) {
  ...
  void resetClusterWeight(unsigned long id, nodeDistanceObject &nodeDistances) {
    //        double weight =
    //        calculateClusterWeight(currentClusters[id].getElements(),
    //        nodeDistances);
  ...
  pair<double, double>
  getModularityScore(boost::dynamic_bitset<unsigned long> elements,
                     graph_undirected_bitset &clusterGraph) {

    double actualEdges = 0;
    double expectEdges = 0;

    for (unsigned long i = elements.find_first(); i < elements.size();
         i = elements.find_next(i)) {
      expectEdges += clusterGraph.getDegree(i);
      for (unsigned long j = elements.find_next(i); j < elements.size();
           j = elements.find_next(j)) {
...
inline unsigned performValidityCheck(currentClusterClassBitset &currentClusters,
  ...
  for (vector<unsigned long>::iterator newClusterIt = newClustersSorted.begin();
    ...
    if (!currentClusters.checkClusterValidity(*newClusterIt)) {
      currentClusters.deleteCluster(
          *newClusterIt, nodeIDsToNames,
          debug); // don't worry about the non-maximal child clusters. Since
                  // they won't pass here
      ++deleted;
    }
  }
...
bool isMinNodeDegreeMet(unsigned cluster1, unsigned cluster2,
  ...
  boost::dynamic_bitset<unsigned long> elements1 =
      currentClusters.getElements(cluster1);
  boost::dynamic_bitset<unsigned long> elements2 =
      currentClusters.getElements(cluster2);
  boost::dynamic_bitset<unsigned long> combination =
      currentClusters.getElements(cluster1);
  boost::dynamic_bitset<unsigned long> joint =
      currentClusters.getElements(cluster1);
  ...
  if (elements1.is_subset_of(elements2) || elements2.is_subset_of(elements1)) {
    return false;
  }
  ...
  if ((numJoint > 1) &&
      (numJoint / numCombined >
       density)) { // this faciliate merge of highly overlapped ones
    for (unsigned i = joint.find_first(); i < joint.size();
         i = joint.find_next(i)) {
    ...
    nfail = 0;
    for (unsigned i = elements2.find_first(); i < elements2.size();
         i = elements2.find_next(i)) {
      if (joint[i]) {
        continue;
      }
    ...
    nfail = 0;
    for (unsigned i = elements1.find_first(); i < elements1.size();
         i = elements1.find_next(i)) {
      if (joint[i]) {
        continue;
      }
      boost::dynamic_bitset<unsigned long> interactorsInCombo = combination;
...
bool isSimilar(unsigned cluster1, unsigned cluster2,
               currentClusterClassBitset &currentClusters,
               graph_undirected_bitset &clusterGraph, double cutoff,
               vector<string> &nodeIDsToNames) {
  boost::dynamic_bitset<unsigned long> elements1 =
      currentClusters.getElements(cluster1);
  boost::dynamic_bitset<unsigned long> elements2 =
  ...
  for (unsigned long i = elements1.find_first(); i < elements1.size();
       i = elements1.find_next(i)) {
    for (unsigned long j = elements2.find_first(); j < elements2.size();
         j = elements2.find_next(j)) {
      if (clusterGraph.isEdge(i, j)) {
        v1 += std::max(1 - clusterGraph.getDegree(i) *
                               clusterGraph.getDegree(j) / (2 * m),
                       0.0);
      }
    }
  }

  for (unsigned long k = combination.find_first(); k < combination.size();
       k = combination.find_next(k)) {
    //            expectEdges2 += clusterGraph.getDegree(k);
    for (unsigned long l = combination.find_next(k); l < combination.size();
         l = combination.find_next(l)) {
      if (clusterGraph.isEdge(k, l)) {
        //                    v2 += std::max(1-clusterGraph.getDegree(k) *
        //                    clusterGraph.getDegree(l)/(2*m), 0.0);
        v2 += abs(1 - clusterGraph.getDegree(k) * clusterGraph.getDegree(l) /
                          (2 * m));
      } else {
        v2 += abs(0 - clusterGraph.getDegree(k) * clusterGraph.getDegree(l) /
                          (2 * m));
      }
    }
  }
...
bool findClustsWithSeed(
  ...
  // to decide if a node is the neighbor of all nodes in seedClust. If true,
  // then this node needs to be merge into seedClust
  ...
  for (unsigned i = 0; i < clusterGraph.numNodes();
       ++i) { // I think here I can only consider affected nodes
    if ((seedClust[i] == 0) &&
  ...
  if (neighborsOfAll.size() > 0) {
    updateCliques(clusterGraph, newClustersToAdd, seedClust, neighborsOfAll,
                  neighborsOfAllBits, 0,
...
void updateClustersWithEdges(vector<pair<unsigned, unsigned>> &edgesToAdd,
  ...
  for (unsigned long edgesToAddCounter = 0;
       edgesToAddCounter != edgesToAdd.size(); ++edgesToAddCounter) {
  ...
  for (unsigned i = 0; i < affectedClusters.size(); ++i) {
    if (affectedClusters[i] && currentClusters.isNew(i)) {
      if (findClustsWithSeed(currentClusters.getElements(i), clusterGraph,
        ...
        currentClusters.deleteCluster(i, nodeIDsToNames,
                                      debug); // set true for now for debugging
        ++deleted;
      }
    }
  }
  ...
  for (unsigned long edgesToAddCounter = 0;
    ...
    if (!findClustsWithSeed(seedClust, clusterGraph, newClustersToAdd,
                            nodeIDsToNames)) { // seedClust is updated inside
                                               // the findClustWithSeed function
  ...
  cout << "# Found " << newClustersToAdd.size() << " new clusters to add"
       << endl;
  for (vector<boost::dynamic_bitset<unsigned long>>::iterator clustersToAddIt =
           newClustersToAdd.begin();
...
bool combineClusters(
    ...
    currentClusterClassBitset &currentClusters,
    graph_undirected_bitset &clusterGraph, vector<string> &nodeIDsToNames,
    nodeDistanceObject &nodeDistances) {
  ...
  sort(clustersToCombine.begin(), clustersToCombine.end(),
       compClustersToCombine); // not sure whether this sort is helpful

  for (vector<pair<pair<unsigned long, unsigned long>, double>>::iterator
           clustersToCombineIt =
               clustersToCombine.begin(); // why is this complaining
    ...
    for (unsigned i = onlyIn1.find_first(); i < onlyIn1.size();
      ...
      newEdgesFrom1To2 -=
          clusterGraph.getInteractors(i); // excluding old edges (THINK ABOUT IT
                                          // IS clusterGraph or clusterGraph)
      for (unsigned j = newEdgesFrom1To2.find_first();
        ...
        if (!edgeWillBeAdded[i][j]) { // don't comment out this line
          edgesToAdd.push_back(
              make_pair(i, j)); // here you are adding all edges!
          edgeWillBeAdded[i][j] = true;
          edgeWillBeAdded[j][i] = true;
          if (edgesToAdd.size() ==
              edgesToAdd.capacity()) { // allocate more memory
...
/**/
bool addMissingEdges(currentClusterClassBitset &currentClusters, double density,
                     double threshold, graph_undirected_bitset &clusterGraph,
                     vector<string> &nodeIDsToNames,
                     nodeDistanceObject &nodeDistances,
                     graph_undirected_bitset &realEdges,
                     bool legacyBeta = false) {

  vector<pair<pair<unsigned long, unsigned long>, double>>
      clustersToCombine; // the bitset is the element after combine
  ...
  for (unsigned long i = 0; i < maxClusterID; ++i) {
    if (currentClusters.isNew(i) && (currentClusters.numElements(i) !=
                                     0)) { //&& //don't exactly why I need this,
                                           //seems to be duplicated to 1,2
  ...
  cout << "# Considering combining " << clustersToCombine.size()
       << " pairs of clusters" << endl;
  if (clustersToCombine.size() > 0) {
    bool realCombine =
        combineClusters(clustersToCombine, currentClusters, clusterGraph,
                        nodeIDsToNames, nodeDistances);
    return realCombine; // if return true, new cluster are created. Should
                        // consider more combine
  }
  return false;
}
...
unsigned getFuzzyClustersBitsetThreshold(
    ...
    vector<validClusterBitset> &validClusters, double alpha, double beta,
    double modular, double zmodular, double stopt, bool legacyBeta) {
  ...
  for (map<string, unsigned>::iterator it = nodeNamesToIDs.begin();
       it != nodeNamesToIDs.end(); ++it) { // what is going on here
  ...
  graph_undirected_bitset clusterGraph(
      numNodes); // all the edges covered by clusters by this time
  graph_undirected_bitset realEdges(
      numNodes); // all the real edges by this time

  //
  // double dt = nodeDistances.sortedDistancesBegin()->second; // Current
  // threshold, starting with the maximum similarity
  double dt = 1.0;
  double last_dt = dt;
  ...
  while ((clusterGraph.numEdges() < 0.5 * totalEdges) &&
         ...
         (dt != stopt)) { // termination conditions
    ...
    addUntil = dt - alpha; // addUntil can cross multiple alpha if there is a
                           // region with low edge density
    while ((distanceIt != nodeDistances.sortedDistancesEnd()) &&
      ...
      realEdges.addEdge(firstNode,
                        secondNode); // add edges to realEdges; this is the only
                                     // place that realEdges change
      ++numRealEdgesAdded;
    ...
    cout << "# Current distance: " << distanceIt->second << "\t"
         << "Add until: " << addUntil << "\t" << endl;
    ...
    cout << "# Number of clusters before merging: " << numClustersBeforeDelete
         << endl;
    ...
    if (beta < 1) { // TODO: this line should be changed as well
      ...
      bool newEdgesAdded =
          addMissingEdges(currentClusters, beta, alpha, clusterGraph,
                          nodeIDsToNames, nodeDistances, realEdges, legacyBeta);

      cout << "# " << ndeleted
           << " clusters deleted in validity check; Now has "
      ...
      while (newEdgesAdded == true) { // recursively merging cliques
        // delete invalid clusters after merging. remember, if a merged clique
        // is deleted, the two original cliques must be returned
        unsigned ndeleted = performValidityCheck(currentClusters, nodeDistances,
                                                 nodeIDsToNames);
        newEdgesAdded = addMissingEdges(currentClusters, beta, alpha,
                                        clusterGraph, nodeIDsToNames,
                                        nodeDistances, realEdges, legacyBeta);
        cout << "# " << ndeleted
             << " clusters deleted in validity check; Now has  "
             << currentClusters.numCurrentClusters() << " clusters" << endl;
      }
    ...
    currentClusters.sortNewClusters(
        newClustersSorted); // sort by size, ascending; this only has new active
                            // clusters

    cout << "# Current number of clusters:"
    ...
    while (newClustersSorted.size() > 0) {
      unsigned long clusterTop =
          newClustersSorted.back(); // think about the order
      newClustersSorted.pop_back();
      if (currentClusters.numElements(clusterTop) ==
          0) { // some clusters are "hold-on" this stage.
      ...
      double uniqueThresh =
          0.1 * pow(currentClusters.getElements(clusterTop).count(),
                    2.0); // soft for small, I think quite strong for big

      if (currentClusters.getNumUniquelyUnexplainedEdges(clusterTop) <
          uniqueThresh) {
        currentClusters.deleteCluster(clusterTop, nodeIDsToNames, debug);
        ++unique_filter;
        continue;
    ...
    //            if (legacyBeta) {
    cout << "# " << unique_filter << " clusters failed the uniqueness filter"
         << endl;
    //            }

    currentClusters.sortNewClusters(tempNewAndValid);

    for (vector<unsigned long>::iterator newValidClusterIt =
             tempNewAndValid.begin();
         newValidClusterIt != tempNewAndValid.end(); ++newValidClusterIt) {
      currentClusters.setClusterValid(*newValidClusterIt, realEdges);
      //                printCluster(currentClusters.getElements(*newValidClusterIt),
      //                nodeIDsToNames); cout <<
      //                currentClusters.getClusterWeight(*newValidClusterIt) <<
      //                "\t" << currentClusters.isNew(*newValidClusterIt) <<
      //                endl;
    }

    for (unsigned long i = 0; i < currentClusters.maxClusterID(); ++i) {
      if ((currentClusters.numElements(i) != 0) && currentClusters.isValid(i) &&
          (!currentClusters.isNew(i))) {
        double clustWeight = currentClusters.getClusterWeight(i);
        pair<double, double> mod = currentClusters.getModularityScore(
            currentClusters.getElements(i),
            realEdges); // this is the modularity after 1 round

        if ((mod.first > modular) && (mod.second > zmodular)) {
          validClusters.push_back(validClusterBitset(
              currentClusters.getElements(i), 0, clustWeight));
          cout << "# Valid cluster:\t";
          ...
          if (legacyBeta) {
            cout << "\t" << clustWeight << "\t" << mod.first << "\t"
                 << mod.second << "\t"
                 << currentClusters.getNumUniquelyUnexplainedEdges(i) << "\t"
                 << endl;
          } else {
            cout << "\t" << clustWeight << "\t" << mod.first << "\t"
                 << mod.second << "\t"
    ...
    cout << "# " << modular_filter << " clusters failed the modularity filter"
         << endl;
    ...
    cout << "# Num edges covered by valid clusters: " << realEdges.numEdges()
         << endl;
    time(&end);
    dif = difftime(end, start);
    cout << "# Time elapsed: " << dif << " seconds" << endl;
...
void constructDAG(graph_undirected &input_graph, DAGraph &ontology,
                  ...
                  double density, double modular, double zmodular, double stopt,
                  bool legacyBeta) {
  // ontology = DAGraph();
  ...
  dagConstruct::getFuzzyClustersBitsetThreshold(
      nodeDistances, geneNamesToIDs, validClusters, threshold, density, modular,
      zmodular, stopt, legacyBeta); // all important stuff in here
